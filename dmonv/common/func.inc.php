<?php
	class jabarsoft{
		...
		function Jurnal2wh($kd="", $fSbb="",$fnobukti="",$tgltrn="",$fNominal="",$fKet="",$fField="",$fValue="",$f2Field="",$f2Value=""){
			...
			$Isi=mysql_fetch_array(mysql_query("select a.nosbb from keu_tblgl as a where left(a.nosbb,".strlen(trim($fSbb)).") ='".trim($fSbb)."' $wh"));			
			$nosbb=$Isi['nosbb'];
			...
			$Qry=mysql_query("select * from `keu_toftrnc` where `nobukti`='{$fnobukti}'");
				if(mysql_num_rows($Qry)>0){
					mysql_query("UPDATE `keu_toftrnc` SET `chguser`='$sUserId', `chgtgl`='$TANGGAL_AKSES' WHERE  `nobukti`='{$fnobukti}';");
					mysql_query("DELETE FROM `keu_toftrnc_detail` WHERE  `nobukti`='{$fnobukti}' AND `noperk`='{$nosbb}';");
					mysql_query("INSERT INTO `keu_toftrnc_detail` (`nobukti`, `noperk`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fnobukti}', '{$nosbb}', '{$fNominal}', '{$fnobukti}{$fValue}{$f2Value}{$fKet}', '{$fValue2}{$f2Value2}');");					
				}else{
					$Qry2=mysql_query("select * from `keu_toftrnh` where `nobukti`='{$fnobukti}'");
					if(!mysql_num_rows($Qry2)){
						mysql_query("INSERT INTO `keu_toftrnc` (`nobukti`, `tgltrn`, `inpuser`, `inptgl`) VALUES ('{$fnobukti}', '{$tgltrn}', '{$sUserId}', '{$TANGGAL_AKSES}');");	
				...
				$CodeLB=3900000000;
				if ($nosbb != $CodeLB){
					$cjml=mysql_num_rows(mysql_query("select * from keu_tblgl as a where a.nosbb ='$nosbb' and a.acctype in ('5','4');"));
					if ($cjml > 0){
						mysql_query("INSERT INTO `keu_toftrnc_detail` (`nobukti`, `noperk`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fnobukti}' , '{$CodeLB}', '{$fNominal}', 'LABA RUGI', '".strtoupper($kd)."  {$nosbb}  LABA RUGI AUTO');");			
					}
				}
			}
		}
...
//			echo "select a.nosbb from keu_tblgl as a where left(a.nosbb,".strlen(trim($fSbb)).") ='".trim($fSbb)."' $wh";
			...
			$Qry=mysql_query("select * from `keu_toftrnc` where `nobukti`='{$fnobukti}'");			
				if(mysql_num_rows($Qry)>0){						
					mysql_query("UPDATE `keu_toftrnc` SET `chguser`='$sUserId', `chgtgl`='$TANGGAL_AKSES' WHERE  `nobukti`='{$fnobukti}';");
					mysql_query("DELETE FROM `keu_toftrnc_detail` WHERE  `nobukti`='{$fnobukti}' AND `noperk`='{$nosbb}';");
					mysql_query("INSERT INTO `keu_toftrnc_detail` (`nobukti`, `noperk`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fnobukti}', '{$nosbb}', '{$fNominal}', '{$fnobukti}{$fValue}{$fValue2}{$fValue3}{$fKet}', '{$fValueF2}{$fValue2F2}{$fValue3F2}');");	
				...
				//	ECHO "KAHIJI - INSERT INTO `keu_toftrnc_detail` (`nobukti`, `noperk`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fnobukti}', '{$nosbb}', '{$fNominal}', '{$fnobukti}{$fValue}{$fValue2}{$fValue3}{$fKet}', '{$fValueF2}{$fValue2F2}{$fValue3F2}');<HR>";
					...
					if(!mysql_num_rows($Qry2)){
						mysql_query("INSERT INTO `keu_toftrnc` (`nobukti`, `tgltrn`, `inpuser`, `inptgl`) VALUES ('{$fnobukti}', '{$tgltrn}', '{$sUserId}', '{$TANGGAL_AKSES}');");	
						mysql_query("DELETE FROM `keu_toftrnc_detail` WHERE  `nobukti`='{$fnobukti}' AND `noperk`='{$nosbb}';");
						mysql_query("INSERT INTO `keu_toftrnc_detail` (`nobukti`, `noperk`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fnobukti}', '{$nosbb}', '{$fNominal}', '{$fnobukti}{$fValue}{$fValue2}{$fValue3}{$fKet}', '{$fValueF2}{$fValue3F2}{$fValue3F2}');");
...
//						ECHO "KADUA - INSERT INTO `keu_toftrnc_detail` (`nobukti`, `noperk`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fnobukti}', '{$nosbb}', '{$fNominal}', '{$fnobukti}{$fValue}{$fValue2}{$fValue3}{$fKet}', '{$fValueF2}{$fValue3F2}{$fValue3F2}');<HR>";
		...
		function Jurnal3whPersen($kd="", $fSbb="",$fnobukti="",$tgltrn="",$fNominal="",$fKet="",$fField="",$fValue="",$fField2="",$fValue2="",$fField3="",$fValue3=""){
			...
			$Isi=mysql_fetch_array(mysql_query("select a.nosbb, a.persen from keu_tblgl as a where left(a.nosbb,".strlen(trim($fSbb)).") ='".trim($fSbb)."' $wh"));			
...
//			echo "select a.nosbb, a.persen from keu_tblgl as a where left(a.nosbb,".strlen(trim($fSbb)).") ='".trim($fSbb)."' $wh";
		...
		function Jurnal($kd="", $fSbb="",$fnobukti="",$tgltrn="",$fNominal="",$fKet="",$fField="",$fValue=""){
			...
			$Qry=mysql_query("select * from `keu_toftrnc` where `nobukti`='{$fnobukti}'");			
				if(mysql_num_rows($Qry)>0){						
					mysql_query("UPDATE `keu_toftrnc` SET `chguser`='$sUserId', `chgtgl`='$TANGGAL_AKSES' WHERE  `nobukti`='{$fnobukti}';");
					mysql_query("DELETE FROM `keu_toftrnc_detail` WHERE  `nobukti`='{$fnobukti}' AND `noperk`='{$nosbb}';");
					mysql_query("INSERT INTO `keu_toftrnc_detail` (`nobukti`, `noperk`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fnobukti}', '{$nosbb}', '{$fNominal}', '{$fnobukti}{$fValue}{$fKet}', '{$fValue2}');");					
				}else{
					$Qry2=mysql_query("select * from `keu_toftrnh` where `nobukti`='{$fnobukti}'");
					if(!mysql_num_rows($Qry2)){
						mysql_query("INSERT INTO `keu_toftrnc` (`nobukti`, `tgltrn`, `inpuser`, `inptgl`) VALUES ('{$fnobukti}', '{$tgltrn}', '{$sUserId}', '{$TANGGAL_AKSES}');");	
		...
		function BukuKas($mSbb="",$kd="", $fSbb="",$fno_transaksi="",$tgl_transaksi="",$fNominal="",$fKet="",$fField="",$fValue=""){
			...
			$Qry=mysql_query("select * from `keu_bukukas` where `no_transaksi`='{$fno_transaksi}'");			
				if(mysql_num_rows($Qry)>0){						

					mysql_query("UPDATE `keu_bukukas` SET `chguser`='$sUserId', `chgtgl`='$TANGGAL_AKSES' WHERE  `no_transaksi`='{$fno_transaksi}';");
					mysql_query("DELETE FROM `keu_bukukas_detail` WHERE  `no_transaksi`='{$fno_transaksi}' AND `nosbb`='{$nosbb}';");
					mysql_query("INSERT INTO `keu_bukukas_detail` (`no_transaksi`, `nosbb`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fno_transaksi}', '{$nosbb}', '{$fNominal}', '{$fno_transaksi}{$fValue}{$fKet}', '{$fValue2}');");					
				}else{
					$Qry2=mysql_query("select * from `keu_toftrnh` where `nobukti`='{$fno_transaksi}'");
					if(!mysql_num_rows($Qry2)){
						mysql_query("INSERT INTO `keu_bukukas` (`no_transaksi`, `tgl_transaksi`, `inpuser`, `inptgl`,`nosbb_k`) VALUES ('{$fno_transaksi}', '{$tgl_transaksi}', '{$sUserId}', '{$TANGGAL_AKSES}', '$mSbb');");	
						mysql_query("DELETE FROM `keu_bukukas_detail` WHERE  `no_transaksi`='{$fno_transaksi}' AND `nosbb`='{$nosbb}';");
						mysql_query("INSERT INTO `keu_bukukas_detail` (`no_transaksi`, `nosbb`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fno_transaksi}', '{$nosbb}', '{$fNominal}', '{$fno_transaksi}{$fValue}{$fKet}', '{$fValue2}');");
					}
				}
...
function BukuKas2wh($mSbb="",$kd="", $fSbb="",$fno_transaksi="",$tgl_transaksi="",$fNominal="",$fKet="",$fField="",$fValue="",$f2Field="",$f2Value=""){
			...
			$Qry=mysql_query("select * from `keu_bukukas` where `no_transaksi`='{$fno_transaksi}'");
				if(mysql_num_rows($Qry)>0){
					mysql_query("UPDATE `keu_bukukas` SET `chguser`='$sUserId', `chgtgl`='$TANGGAL_AKSES' WHERE  `no_transaksi`='{$fno_transaksi}';");
					mysql_query("DELETE FROM `keu_bukukas_detail` WHERE  `no_transaksi`='{$fno_transaksi}' AND `nosbb`='{$nosbb}';");
					mysql_query("INSERT INTO `keu_bukukas_detail` (`no_transaksi`, `nosbb`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fno_transaksi}', '{$nosbb}', '{$fNominal}', '{$fno_transaksi}{$fValue}{$f2Value}{$fKet}', '{$fValue2}{$f2Value2}');");					
				}else{
					$Qry2=mysql_query("select * from `keu_toftrnh` where `nobukti`='{$fno_transaksi}'");
					if(!mysql_num_rows($Qry2)){
						mysql_query("INSERT INTO `keu_bukukas` (`no_transaksi`, `tgl_transaksi`, `inpuser`, `inptgl`, `nosbb_k`) VALUES ('{$fno_transaksi}', '{$tgl_transaksi}', '{$sUserId}', '{$TANGGAL_AKSES}', '$mSbb');");	
		...
		function BukuKas3wh($mSbb="",$kd="", $fSbb="",$fno_transaksi="",$tgl_transaksi="",$fNominal="",$fKet="",$fField="",$fValue="",$fField2="",$fValue2="",$fField3="",$fValue3=""){
			...
			$Qry=mysql_query("select * from `keu_bukukas` where `no_transaksi`='{$fno_transaksi}'");			
				if(mysql_num_rows($Qry)>0){						
					mysql_query("UPDATE `keu_bukukas` SET `chguser`='$sUserId', `chgtgl`='$TANGGAL_AKSES' WHERE  `no_transaksi`='{$fno_transaksi}';");
					mysql_query("DELETE FROM `keu_bukukas_detail` WHERE  `no_transaksi`='{$fno_transaksi}' AND `nosbb`='{$nosbb}';");
					mysql_query("INSERT INTO `keu_bukukas_detail` (`no_transaksi`, `nosbb`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fno_transaksi}', '{$nosbb}', '{$fNominal}', '{$fno_transaksi}{$fValue}{$fValue2}{$fValue3}{$fKet}', '{$fValueF2}{$fValue2F2}{$fValue3F2}');");	
				}else{
					...
					if(!mysql_num_rows($Qry2)){
						mysql_query("INSERT INTO `keu_bukukas` (`no_transaksi`, `tgl_transaksi`, `inpuser`, `inptgl`,`nosbb_k`) VALUES ('{$fno_transaksi}', '{$tgl_transaksi}', '{$sUserId}', '{$TANGGAL_AKSES}', '$mSbb');");	
						mysql_query("DELETE FROM `keu_bukukas_detail` WHERE  `no_transaksi`='{$fno_transaksi}' AND `nosbb`='{$nosbb}';");
						mysql_query("INSERT INTO `keu_bukukas_detail` (`no_transaksi`, `nosbb`, `".$kd."`, `keterangan`, `nama`) VALUES ('{$fno_transaksi}', '{$nosbb}', '{$fNominal}', '{$fno_transaksi}{$fValue}{$fValue2}{$fValue3}{$fKet}', '{$fValueF2}{$fValue3F2}{$fValue3F2}');");
...
//			strlen($text)
		...
		function sensor($teks){
			$w = mysql_query("SELECT * FROM t_katajelek");
		...
		function Login()
		{
			...
			{				
				$Qry = mysql_query("SELECT * FROM __t_users WHERE username='$uLogin' AND `password`='$pLogin' AND blokir='N'");
				...
				{						
					$r=mysql_fetch_array($Qry);$UserID=$r['username'];
				...
				{	
					...
					mysql_query("UPDATE __t_users SET id_session='$sid_baru' WHERE username='$UserID'");			
		...
		{
			global $Dir;
			$ph = mysql_fetch_array(mysql_query("select $fphoto from $tabel where $fid='$id'"));
		...
		function KopSurat($KDTAHUNAJARAN="",$Judul="")
		{
			...
			$HKop1=mysql_fetch_array(mysql_query("select * from r_header"));
			$HKop2=mysql_fetch_array(mysql_query("select NM_SEKOLAH, JALAN, KD_POS, KD_AREA, NO_TELP, NO_FAX, EMAIL, WEBSITE  from t_sekolah_identitas where KD_TAHUN_AJARAN='$KDTAHUNAJARAN' limit 1"));
		...
		{	$IsiSliderHeader="";
			$QHeader=mysql_query("select * from t_header where aktif='Y' order by id_header  desc");
			while($IsiHeader=mysql_fetch_array($QHeader))
			{
				$IsiSliderHeader.="
					<li>
	...
	/////////////////////////////////////////////////// fungsi thumb /////////////////////////////////////////
			...
			function CreateTable($namatable="",$Query=""){
				$Qry=mysql_query("SHOW CREATE TABLE $namatable;");
				if(@!mysql_num_rows($Qry)>0)
				{mysql_query($Query);}
			}
	...
	class pageNavi{
		...
		function buatHalaman(){
			...
			$this->Qry = mysql_query($this->Query." limit $Mulai,{$this->Banyak}");
			$this->Jumlah = mysql_num_rows(mysql_query($this->Query));
