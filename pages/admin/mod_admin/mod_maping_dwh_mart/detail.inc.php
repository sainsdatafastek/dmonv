<?php
	...
	switch($Aksi)
	{
		case "Simpan";
			...
			{	
				$Qry=_mysql_query("select * from reg_maping_dwh_to_mart where id_maping='$id'");
				if(_mysql_num_rows($Qry)>0)					
				{$QSimpan="UPDATE `reg_maping_dwh_to_mart` SET `id_dbname_sumber`='{$id_dbname_sumber}', `id_dbname_dwh`='{$id_dbname_dwh}', `id_query`='{$id_query}', `id_table`='{$id_table}',  `tanggal_update`='{$TANGGAL_AKSES}', `user_update`='{$sUserId}' WHERE  `id_maping`='{$id}';";}	
				else
				{$QSimpan="INSERT INTO `reg_maping_dwh_to_mart` (`id_dbname_sumber`, `id_dbname_dwh`, `id_query`, `id_table`, `tanggal_insert`, `user_create`) VALUES ('{$id_dbname_sumber}', '{$id_dbname_dwh}', '{$id_query}', '{$id_table}', '{$TANGGAL_AKSES}', '{$sUserId}');";}
		...
		break;
		case "Lihat":
			$Func->ambilData("select * from `reg_maping_dwh_to_mart` where `id_maping`='".$id."';");
		break;
		case "Hapus":
			_mysql_query("DELETE FROM `reg_maping_dwh_to_mart` where `id_maping`='{$id}';");
			$Func->kosongkanData('reg_maping_dwh_to_mart');
			echo "<script>parent.location='index.php?Pg=$Pg&Pr=$Pr&Sb=$Sb';</script>";	
		break;
	}
	...
	$Main->Isi="
			...
			<tr>
				<td>Database DWH</td>
				<td> ".$Func->cmbQuery("id_dbname_sumber",$id_dbname_sumber,"select id_dbname, dbname from reg_db_sumber where left(dbname,4)='dwh_'","onchange=\"Fm2.Aksi.value='BknSimpan';submitForm('#Fm2', '#faceboxisi', Fm2.action);\"")."</td>
				<td>Query DWH</td>
			...
			<tr>
				<td>Database Mart</td>
				<td> ".$Func->cmbQuery("id_dbname_dwh",$id_dbname_dwh,"select a.id_dbname, a.dbname from reg_db_mart as a ","onchange=\"Fm2.Aksi.value='BknSimpan';submitForm('#Fm2', '#faceboxisi', Fm2.action);\"")."</td>
				<td>Table Mart</td>
				<td> ".$Func->cmbQuery("id_table",$id_table,"select a.id_table, a.table_name from reg_db_mart_table as a inner join reg_db_mart as b on a.dbname=b.dbname where b.id_dbname='{$id_dbname_dwh}'")."</td>
			</tr>
