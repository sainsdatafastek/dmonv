<?php
	...
	if(!empty($id_maping)){
		...
		switch($Action){
			...
			case "Simpan":
				...
				foreach ($field_sumber as $k=>$v) {
					$QCek=_mysql_query("select * from reg_maping_dwh_to_mart_field where id_maping_fields='{$id_maping_fields[$k]}' and id_maping='$id_maping'");
					
					if(_mysql_num_rows($QCek)>0){
						_mysql_query("UPDATE `reg_maping_dwh_to_mart_field` SET `field_sumber`='".trim($field_sumber[$k])."', `field_dwh`='".trim($field_dwh[$k])."', param='".$param[$k]."' WHERE  `id_maping_fields`='{$id_maping_fields[$k]}'and id_maping='$id_maping';");
					}else{
						_mysql_query("INSERT INTO `reg_maping_dwh_to_mart_field` (`id_maping`, `field_sumber`, `field_dwh`,`param`) VALUES ('{$id_maping}', '".trim($field_sumber[$k])."', '".trim($field_dwh[$k])."', '".$param[$k]."');");					
					}
		...
		/**/
			/// TABLES
		///// PARAMETER AND QUERY GRID
		$Func->ambilData("select concat('`',b.dbname,'`.`',c.table_name,'`') as tbldwh ,b.dbname as dbdwh, c.table_name as table_dwh, e.dbname as dbsumber, f.query_name as query_sumber, f.query_script, e.hostname, e.username, e.`password` from reg_maping_dwh_to_mart as a inner join reg_db_mart as b on a.id_dbname_dwh=b.id_dbname inner join reg_db_mart_table as c on a.id_table = c.id_table inner join reg_db_sumber as e on a.id_dbname_sumber= e.id_dbname inner join reg_db_sumber_query as f on a.id_query  = f.id_query where a.id_maping ='{$id_maping}'");
