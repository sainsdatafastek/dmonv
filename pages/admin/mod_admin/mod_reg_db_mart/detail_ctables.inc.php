<?php
	...
	switch($Aksi)
	{
		case "Simpan";
			...
			{	
				$Qry=_mysql_query("select * from `reg_db_mart_table` where `id_table`='$id'");
				if(_mysql_num_rows($Qry)>0)					
				{

					$QSimpan="UPDATE `reg_db_mart_table` SET `table_name`='{$table_name}', `dbname`='{$dbnameid}', `tanggal_update`='{$TANGGAL_AKSES}', `user_update`='{$sUserId}' WHERE  `id_table`='{$id}';";
				...
				if( $QTSimpan){
					$QSimpan="INSERT INTO `reg_db_mart_table` (`table_name`, `dbname`, `tanggal_insert`, `user_create`) VALUES ('{$table_name}', '{$dbnameid}', '{$TANGGAL_AKSES}', '{$sUserId}');";
					$Simpan=_mysql_query( $QSimpan );
		...
		break;
		case "Lihat":
			$Func->ambilData("select * from `reg_db_mart_table` where `id_table`='".$id."';");
			$Func->ambilData("SELECT count(*) as jcol FROM information_schema.columns WHERE table_name = '{$table_name}' and TABLE_SCHEMA='{$dbname}'");
			$dbnameid=$dbname;
		break;
		case "Hapus":
			$Func->ambilData("select dbname, table_name from `reg_db_mart_table` where `id_table`='".$id."';");
			_mysql_query("DROP TABLE IF EXISTS `".$dbname."`.`".$table_name."`;");
			_mysql_query("DELETE FROM `reg_db_mart_table` where `id_table`='{$id}';");
			echo "<script>parent.location='index.php?Pg=$Pg&Pr=$Pr&Sb=$Sb&dbname=$dbnameid';</script>";	
		break;
	}
	
	if(empty($id)&&empty($table_name))
	{$Func->kosongkanData('reg_db_mart_table');}
	
	if($jcol>0){
		$i=1;

		$Qry=_mysql_query("SELECT COLUMN_NAME, DATA_TYPE, replace(replace(replace(COLUMN_TYPE,DATA_TYPE,''),'(',''),')','') as LengthSet FROM information_schema.columns WHERE table_name = '{$table_name}' and TABLE_SCHEMA='{$dbnameid}'");
