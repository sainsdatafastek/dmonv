<?php
	...
	switch($Aksi)
	{
		case "Simpan";
			if(!empty($dbname))
			{	
				$Qry=_mysql_query("select * from reg_db_mart where id_dbname='$id'");
				if(_mysql_num_rows($Qry)>0)					
				{$QSimpan="UPDATE `reg_db_mart` SET `dbname`='".$TextTemp."{$dbname}', `tanggal_update`='{$TANGGAL_AKSES}', `user_update`='{$sUserId}' WHERE  `id_dbname`='{$id}';";}	
				else
				{$QSimpan="INSERT INTO `reg_db_mart` (`dbname`, `tanggal_insert`, `user_create`) VALUES ('".$TextTemp."{$dbname}', '{$TANGGAL_AKSES}', '{$sUserId}');";}
		...
		break;
		case "Lihat":
			$Func->ambilData("select * from `reg_db_mart` where `id_dbname`='".$id."';");
		break;
		case "Hapus":
			$Func->ambilData("select dbname from `reg_db_mart` where `id_dbname`='".$id."';");
			_mysql_query("DROP DATABASE IF EXISTS `".$TextTemp."{$dbname}`;");
			_mysql_query("DELETE FROM `reg_db_mart` where `id_dbname`='{$id}';");
			$Func->kosongkanData('reg_db_mart');
			echo "<script>parent.location='index.php?Pg=$Pg&Pr=$Pr&Sb=$Sb';</script>";	
		break;
	}
	
	if(empty($id)){$Func->kosongkanData('reg_db_mart');}
