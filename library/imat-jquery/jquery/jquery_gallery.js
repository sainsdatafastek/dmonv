			...
			buildPager: function(pager) {
				...
				for (i=this.currentPage-3; i<=this.currentPage+3; i++) {
					...
					if (i == this.currentPage)
						if(this.settings.pageNumber==true){pager.append('<span class="current">'+pageNum+'</span>');}
						//pager.append('<span class="current">'+pageNum+'</span>');
					else if (i>=0 && i<this.numPages) {
						var imageIndex = i*this.settings.numThumbs;
						if(this.settings.pageNumber==true){pager.append('<a rel="history" href="#'+this.data[imageIndex].hash+'" title="'+pageNum+'">'+pageNum+'</a>');}
