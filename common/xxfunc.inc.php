...
//			strlen($text)
		...
		function kosongkanData($tabel)
		{
			...
			while ($Hasil=__mysql_fetch_row($Query))
			{
				$Field = $Hasil[0];
				global $$Field;
				$$Field = "";
		...
		function ambilData($query)
		{
			...
			while ($Field=__mysql_fetch_field($Query1))
			{
				$NmField = $Field->name;
				global $$NmField;
		...
		function rdAktif($name='txtField',$value='',$param='')
		{
			...
			$Input  = "<input type=\"radio\" name=\"$name\" value='Y' $SelY $param> Y";
			$Input .= "&nbsp;<input type=\"radio\" name=\"$name\" value='N' $SelN $param> N ";
			return $Input;
		}
